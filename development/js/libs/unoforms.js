    function getTagName(elem) {
        ...
        if (inputTag != 'select') {inputTag = $(elem).attr('type')}
        if (inputTag == 'select') {inputTag = 'select'}
        if ($(elem).attr('multiple')) {inputTag = 'multiple'}
    ...
    function insert_addon(elem) {
        ...
        $(elem).parent('label').addClass('field');
    ...
    function update_field(elem) {
        ...
        if ($(elem).attr('disabled')) {addon.addClass('disabled')} else {addon.removeClass('disabled')}
        if ($(elem).attr('readonly')) {addon.addClass('readonly')} else {addon.removeClass('readonly')}
        if ($(elem).attr('checked'))  {addon.addClass('checked')}  else {addon.removeClass('checked')}
        ...
        if (elemType == 'radio'){
            $('input[type="radio"]').each(function() {                 
                if (!$(this).attr('checked')) {$(this).prev('.addon').removeClass('checked')}
            });
        }
    }
    ...
    function init_form() {
        $('input, select').each(function() { 
            var thisParent = $(this).parent('label').hasClass('field');
